...
- **spoc:systematic-debugging** — When performance issue is actually a bug (infinite loop, memory leak from logic error)
- **spoc:test-driven-development** — For writing regression benchmarks that catch future performance degradation
- **spoc:verification-before-completion** — Verify optimization holds under production-like conditions before claiming done
