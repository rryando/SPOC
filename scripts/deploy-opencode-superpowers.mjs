#!/usr/bin/env node
// Deploy opencode superpowers bundle from repo to user config.
//
// Direction: repo → config ONLY. Never writes config → repo.
//
// Env vars:
//   DEPLOY_BUNDLE_ROOT  — override bundle root (default: opencode/superpowers)
...
async function main() {
  ...
  // Skills: bundle skills/<name>/<file> → config skills/superpowers/<name>/<file>
