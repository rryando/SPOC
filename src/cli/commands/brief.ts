...
defineCommand({
  ...
  params: {
    ...
    audience: { type: "string", description: "Knowledge audience filter (default: orchestrator)", enum: ["orchestrator", "implementer", "designer"] },
    full: { type: "boolean", description: "Include done tasks/plans (default: false — only active items)" },
  },
  handler: handleBrief,
});
...
async function handleBrief(params: Record<string, unknown>, flags: CommandFlags): Promise<CLIResult> {
  ...
  if (params.audience && !(KNOWLEDGE_AUDIENCES as readonly string[]).includes(params.audience as string)) {
    return failure("invalid_enum", `Invalid audience "${params.audience}". Valid: ${KNOWLEDGE_AUDIENCES.join(", ")}`);
  }
  ...
  const topKnowledge = filtered.slice(0, 5).map((e) => ({ id: e.id, title: e.title, kind: e.kind }));
...
async function resolveProject(rawArg: string | undefined): Promise<ResolveSuccess | ResolveFailure> {
  ...
  if (!rawArg) {
    ...
    if (!existsSync(projMetaPath)) {
      return { ok: false, result: failure(ERROR_CODES.PROJECT_NOT_FOUND, `Project "${rawArg}" not found in SPOC`) };
    }
    try {
      ...
      if (paths.length === 0) {
        return { ok: false, result: failure("no_workspace_paths", `Project "${rawArg}" has no workspace paths configured`) };
      }
      const first = paths[0];
      queryPath = first.startsWith("~") ? resolve(homedir(), first.slice(2)) : resolve(first);
    } catch {
      return { ok: false, result: failure("read_error", `Could not read project metadata for "${rawArg}"`) };
    }
  } else {
  ...
  if (matchResult.kind === "none") {
    return { ok: false, result: failure("no_match", `No project found matching path "${queryPath}"`) };
  }
