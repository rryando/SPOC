...
5. **Summarize the agreed plan** — scope, key decisions, trade-offs accepted, assumptions made, and proposed tasks/plan structure. Silently load the \`to-diagram\` skill (do not narrate the load or its conventions) and generate a Mermaid plan diagram. Use \`flowchart TD\` for task dependency graphs, \`stateDiagram-v2\` for lifecycle phases. All nodes start as \`:::backlog\` at plan creation time. Write the diagram to the \`.mmd\` file at \`~/.spoc/projects/<slug>/plans/<plan-id>.diagram.mmd\`. The plan body references this file by path — no embedded Mermaid in the plan body. If the visual companion (brainstorming server) is available, write an HTML wrapper and present the rendered diagram URL to the user for review. Fall back to inline Mermaid in chat only if the visual companion is unavailable. Present the plan summary as a numbered list.
...
6. Keep docs in sync with \`update_project_doc\`:
    ...
    - dependencies: record relationship changes
     - diagram: the **orchestrator** owns diagram updates — implementation sub-agents never touch diagrams. When a task status changes, update the corresponding node's \`:::className\` in the plan's \`.diagram.mmd\` file (\`~/.spoc/projects/<slug>/plans/<plan-id>.diagram.mmd\`) at the **write-gate** (step 9), alongside task status updates. For status-only changes, edit the \`.mmd\` file and update \`:::className\` assignments only — topology stays. If tasks were added, removed, or renamed during this execution session, regenerate the \`.mmd\` file from current plan structure — load \`to-diagram\` skill silently for the regeneration decision tree. Include diagram regeneration in the write-gate summary (step 9) when scope changed. If the visual companion is running, the user can view the updated diagram there.
...
4. Audit surfaces the sub-agent should cover:
   ...
   - **knowledge/**: Are entries still accurate? Any missing entries for recent discoveries?
    ...
    - **plans/ diagrams**: For each plan, audit its associated \`.diagram.mmd\` file (\`~/.spoc/projects/<slug>/plans/<plan-id>.diagram.mmd\`) against task metadata. Check for: classDef status mismatch (node shows \`:::done\` but task is \`in_progress\`), phantom nodes (diagram node has no corresponding task), missing nodes (task exists but diagram has no corresponding node), topology mismatch (edges don't match task dependencies). Load \`to-diagram\` skill for drift detection rules. If drift found, regenerate the \`.mmd\` file from current task metadata.
...
- **plans/**: Structured plan records for multi-step feature work (the plan surface). Each plan has an associated \`.diagram.mmd\` file (\`~/.spoc/projects/<slug>/plans/<plan-id>.diagram.mmd\`) as a visual companion rendered via the visual companion (brainstorming server) and readable by agents for quick orientation.
