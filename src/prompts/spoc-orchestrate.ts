...
5. **Summarize the agreed plan** — scope, key decisions, trade-offs accepted, assumptions made, and proposed tasks/plan structure. Silently load the \`to-diagram\` skill (do not narrate the load or its conventions) and generate a Mermaid plan diagram (required in the plan body under \`## Diagram\`). Use \`flowchart TD\` for task dependency graphs, \`stateDiagram-v2\` for lifecycle phases. All nodes start as \`:::backlog\` at plan creation time. Write the diagram to the plan body. If the system prompt contains a \`<spoc_dashboard>\` tag, present the dashboard URL to the user for review. If no dashboard is running, start it silently via \`bash ~/.config/opencode/skills/superpowers/spoc-dashboard/start-server.sh\` and present the URL. Fall back to inline Mermaid in chat only if the dashboard cannot start. Present the plan summary as a numbered list.
